...
function cli:env_init()
	...
	self.lazy_dir = helper.path_join(self.config_path, "lazy")
...
function cli:get_all_packages()
	...
	local p = io.open('find "' .. cli.module_path .. '" -type f')
	if not p then
		return
	end
	...
	local lazy_keyword = { "keys", "ft", "cmd", "event", "lazy" }
	local function generate_node(tbl, list)
		...
		if tbl.dependencies then
			for _, v in pairs(tbl.dependencies) do
				...
				list[v[1]] = { from_depend = true, load_after = node }
